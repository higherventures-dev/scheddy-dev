...
export default function BookingActionDrawer({
  ...
  // Hydrate only when open AND booking exist (prevents null access)
  React.useEffect(() => {
  ...
  // If we somehow opened View/Edit/Cancel without a booking, show a gentle fallback
  ...
  async function handleEditSave(e: React.FormEvent) {
    ...
    try {
      ...
      const start = selectedDate
        ? new Date(`${selectedDate.toISOString().split('T')[0]}T${selectedTime || '09:00'}`)
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-lg">
        ...
        {!missingBooking && (action === 'View' || action === 'Edit') && (
          <form className="mt-5 space-y-5" onSubmit={handleEditSave}>
            {/* Service */}
