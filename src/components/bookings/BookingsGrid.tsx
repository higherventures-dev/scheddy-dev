...
export default function BookingsGrid({ bookings, cancelAction }: Props) {
  if (!bookings?.length) {
            ...
            <div className="bg-[#3A3A3A] p-4 rounded-lg">
              <div className="grid grid-cols-6 gap-1">
                ...
                <div className="col-span-2 pr-1 text-right flex items-center justify-end gap-2">
                  ...
                  >
                    <Image src={bookingStatus.image} alt={bookingStatus.name} width={16} height={16} className="mr-1" />
                    <span className="text-[90%]">{bookingStatus.name}</span>
                  </span>

                  {canCancel && (
                    <form action={cancelAction}>
                      {/* Ensure the name matches what you read in the action */}
                      <input type="hidden" name="bookingId" value={String((b as any).id)} />
                      <Button type="submit" size="sm" variant="destructive" className="h-6 px-2 py-0 text-xs">
                        Cancel
                      </Button>
                    </form>
                  )}
              ...
              <div className="grid grid-cols-6 gap-1 pt-4">
                <div className="col-span-1 rounded border border-gray-600 ">
                  <Image src="/assets/images/product.png" alt="" width={20} height={20} className="mx-auto pt-3 pb-3" />
                </div>
